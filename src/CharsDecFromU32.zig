...
pub const MAX_LEN    = "4294967295".len; // UINT32_MAX
...
pub fn set          (self: *Self, _num: u64) void {
    self.reset();
...
pub fn get          (self: *Self) error{Unexpected} ![]u8 {
...
pub fn getDigit     (self: *Self, wigth: usize) error{ Overflow, Unexpected }![]u8 {
    if (self.start > self.end) return error.Unexpected;
    if (self.end > last_digit) return error.Overflow;
    if (wigth > MAX_LEN) return error.Overflow;
    return self.buff[MAX_LEN - wigth .. MAX_LEN];
}
pub fn getMinWidth  (self: *Self, wigth: usize) error{ Overflow, Unexpected }![]u8 {
